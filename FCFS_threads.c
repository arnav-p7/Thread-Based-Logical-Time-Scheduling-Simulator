/*
FCFS_threads.c - Thread-based FCFS simulation (logical time, no sleep)
UPGRADED WITH:
- IO Burst support (io_burst, doing_io)
- results.txt export for chart visualization
*/
...
static void *worker(void *arg) {
    ...
    while (1) {
        ...
        printf("[FCFS] Process %d ran from %d to %d (exec=%d)\n",
               p->pid, s, c, et);
...
void run_FCFS_from_main(int n, int *pid, int *arrival, int *burst,
                        int *priority, int *io_burst, int quantum) {

    (void)quantum;
    printf("\n==== FCFS (threads, logical-time, IO-enabled) ====\n");
    ...
    while (completed_count < P) {
        ...
        /* FIND NEXT PROCESS (earliest arrival) */
        int idx = -1;
        int earliest = 1 << 30;
